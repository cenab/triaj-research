...
try:
    ...
    print("Warning: imblearn not installed; SMOTE balancing disabled.")
import time
...
try:
    ...
    print("Warning: kagglehub not installed; automatic Kaggle download disabled.")
try:
    import pyreadr  # type: ignore
except ImportError:  # pragma: no cover - optional dependency
    pyreadr = None  # type: ignore
    print("Warning: pyreadr not installed; RData ingestion disabled.")
import warnings
...
def load_kaggle_triage_data():
    ...
    if os.path.exists(csv_path):
        ...
        if kagglehub is None or pyreadr is None:
            raise ImportError(
                "advanced Kaggle feature engineering requires `kagglehub` and `pyreadr`."
                " Install them or provide `src/kaggle_triage_data.csv`."
            )
...
def advanced_kaggle_feature_engineering(df):
    ...
    # 1. Target variable: ESI (Emergency Severity Index) — use full 5 classes
    # Ensure ESI is numeric and drop rows with missing ESI
    ...
    for orig_col, new_col in vital_cols.items():
        if orig_col in df.columns:
            ...
            # Fill missing with median by ESI class (5-class target)
            for esi_class in df['esi'].unique():
                mask = (df['esi'] == esi_class) & df[new_col].isna()
                median_val = df[df['esi'] == esi_class][new_col].median()
    ...
    for feature in all_features:
        if feature in df.columns:
            ...
            for esi_class in df['esi'].unique():
                mask = (df['esi'] == esi_class) & df[feature].isna()
                if mask.sum() > 0:
                    median_val = df[df['esi'] == esi_class][feature].median()
                    if pd.notna(median_val):
...
class AdvancedHierarchicalTriageModel(nn.Module):
    ...
    def __init__(self, num_vital_features=8, num_symptom_features=5, 
                 ...
                 num_lab_features=8, num_interaction_features=2, num_classes=5):
        super().__init__()
...
class AdvancedClinicalSafetyLoss(nn.Module):
    ...
    def __init__(self, class_weights, critical_miss_penalty=50.0, alpha=0.25, gamma=2.0):
        ...
        # Dynamic penalty matrix based on number of classes
        self.num_classes = int(class_weights.shape[0])
        self.base_under_penalty = 5.0
        self.base_over_penalty = 1.0
        # Treat top 2 severity levels as "critical band" (e.g., ESI 1-2)
        self.critical_threshold = max(1, self.num_classes - 2)
        
        penalty = []
        for i in range(self.num_classes):  # true class
            row = []
            for j in range(self.num_classes):  # predicted class
                if j < i:
                    # Under-triage: harsher penalty grows quadratically with distance
                    dist = i - j
                    val = (dist ** 2) * self.base_under_penalty
                elif j > i:
                    # Over-triage: milder linear penalty
                    dist = j - i
                    val = dist * self.base_over_penalty
                else:
                    val = 0.0
                row.append(val)
            penalty.append(row)
        self.penalty_matrix = torch.tensor(penalty, dtype=torch.float32)
    ...
    def forward(self, outputs, targets):
        ...
        penalty_mat = self.penalty_matrix.to(outputs.device)
        safety_penalty_vals = penalty_mat[targets, pred_classes]
        safety_penalty = safety_penalty_vals.mean()
        
        # Extra penalty for critical misses (top 2 severity levels)
        critical_mask = (targets >= self.critical_threshold)
        if critical_mask.sum() > 0:
            critical_preds = pred_classes[critical_mask]
            critical_misses = (critical_preds < self.critical_threshold).sum().float()
    ...
    def _calculate_clinical_safety(y_true, y_pred):
        ...
        if len(y_true) == 0:
            return {
                ...
                'critical_correctly_identified': 0
            }
        
        # Under-triage: Predicting lower acuity than actual (with ascending severity encoding)
        ...
        # Critical band: top 2 severity levels
        num_labels = int(max(np.max(y_true), np.max(y_pred))) + 1
        critical_threshold = max(1, num_labels - 2)
        critical_cases_mask = (y_true >= critical_threshold)
        critical_cases = int(np.sum(critical_cases_mask))
        if critical_cases > 0:
            critical_detected = int(np.sum(critical_cases_mask & (y_pred >= critical_threshold)))
            critical_sensitivity = critical_detected / critical_cases
            critical_missed = int(np.sum(critical_cases_mask & (y_pred < critical_threshold)))
        ...
        return {
            ...
            'total_critical_cases': critical_cases,
            'critical_correctly_identified': critical_detected
        }
...
def run_advanced_kaggle_enhanced_final_fix():
    ...
    # Show original ESI distribution (1=most critical, 5=least urgent)
    if 'esi' in df_enhanced.columns:
        print(f"Target distribution (ESI values): {df_enhanced['esi'].value_counts().sort_index()}")
    ...
    # Encode to ascending severity labels: 0=ESI5 (least urgent) ... 4=ESI1 (most critical)
    esi_vals = pd.to_numeric(df_enhanced['esi'], errors='coerce').astype(int).clip(lower=1, upper=5)
    y = (5 - esi_vals.values).astype(int)
    ...
    print(f"Final encoded target distribution (0..4 => ESI5..ESI1): {np.bincount(y)}")
    ...
    if SMOTE is None:
        print("SMOTE unavailable; using original training distribution.")
    ...
    num_classes_training = int(np.max(y_train_balanced)) + 1
    model = AdvancedHierarchicalTriageModel(
        ...
        num_classes=num_classes_training
    ).to(device)
    ...
    for epoch in range(epochs):
        ...
        # Calculate critical sensitivity for top-2 severity levels (ESI1-2)
        ct = max(1, num_classes_training - 2)
        targets_arr = np.array(all_targets)
        preds_arr = np.array(all_preds)
        critical_cases = np.sum(targets_arr >= ct)
        if critical_cases > 0:
            critical_detected = np.sum((targets_arr >= ct) & (preds_arr >= ct))
    ...
    # Class names correspond to encoded labels 0..4 => ESI5..ESI1
    metrics = EnhancedClinicalMetrics.calculate_comprehensive_clinical_metrics(
        all_targets, all_predictions, class_names=['ESI5', 'ESI4', 'ESI3', 'ESI2', 'ESI1']
    )
