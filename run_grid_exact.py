...
def main():
    ...
    cols = [
        ...
        "overall_fairness_score", "max_fairness_gap", "conformal_tau", "conformal_alpha",
        ...
        for i, task in enumerate(tasks, start=1):
            ...
            if ttype == "centralized":
                ...
                for k, v in base_kwargs.items():
                    if _supports(sig, k):
                        kwargs[k] = v
                if _supports(sig, "use_conformal") and "use_conformal" not in kwargs:
                    kwargs["use_conformal"] = True
                if _supports(sig, "conformal_alpha") and "conformal_alpha" not in kwargs:
                    kwargs["conformal_alpha"] = 0.05
                ...
                reliability = report.get("reliability_metrics", {}) or {}
                fairness = report.get("fairness_metrics", {}) or {}
                overall_fairness = fairness.get("overall_fairness_score") if fairness else None
                fairness_violations = fairness.get("fairness_violations", []) if fairness else []
                max_fairness_gap = None
                if fairness_violations:
                    try:
                        max_fairness_gap = max(float(v.get("difference", 0.0)) for v in fairness_violations)
                    except Exception:
                        max_fairness_gap = None
                conformal_info = report.get("conformal") or report.get("thresholds", {}).get("selection", {}) or {}
                conformal_tau = conformal_info.get("tau")
                conformal_alpha = conformal_info.get("alpha")
                ...
                row = {
                    ...
                    "conformal_tau": float(conformal_tau) if conformal_tau is not None else np.nan,
                    "conformal_alpha": float(conformal_alpha) if conformal_alpha is not None else None,
                ...
                jf.write(json.dumps({
                    ...
                    "selection_criterion": report.get("selection_criterion", {}),
                    "artifacts": {
